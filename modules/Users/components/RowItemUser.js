...
export default function RowItemUser({ data, updateActive }) {
    ...
    const { checkNull, avatarUser, getDate, isActive } = displayHelpers;
    ...
    return (
        <tr>
            ...
            <td className="px-6 align-middle 2xl:text-sm text-xs xl:text-xs text-center whitespace-nowrap p-4">{checkNull(phone)}</td>
            <td className="px-6 align-middle 2xl:text-sm xl:text-xs text-xs text-center whitespace-nowrap p-4">{checkNull(email)}</td>
            <td className="px-6 align-middle 2xl:text-sm xl:text-xs text-xs text-center whitespace-nowrap p-4">{getDate(createdAt)}</td>
            <td className="px-6 align-middle 2xl:text-sm xl:text-xs text-xs text-center whitespace-nowrap p-4">{isRole(role)}</td>
            <td className="px-6 align-middle 2xl:text-sm xl:text-xs text-xs text-center whitespace-nowrap p-4">{isActive(active)}</td>
            <td className="px-6 align-middle 2xl:text-sm xl:text-xs text-xs text-center whitespace-nowrap p-4">{activeUser(active)}</td>
        </tr>
    );
}
